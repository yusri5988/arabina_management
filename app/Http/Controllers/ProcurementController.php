...
class ProcurementController extends Controller
{
    ...
    {
        ProcessLogger::start('Procurement', 'Create Order', 'validate_input');
        ...
        ProcessLogger::success('Procurement', 'Create Order', 'save_result', [
            'orders_count' => count($createdOrders),
            'first_order_id' => $createdOrders[0]->id ?? null,
            'scope' => $scope
        ], ProcurementOrder::class, $createdOrders[0]->id ?? null);
    ...
    {
        ProcessLogger::start('Procurement', 'Receive Order', 'validate_input', [
            'order_id' => $order->id,
            'order_code' => $order->code
        ], ProcurementOrder::class, $order->id);
        ...
        ProcessLogger::success('Procurement', 'Receive Order', 'save_result', [
            'order_id' => $order->id,
            'status' => $order->status
        ], ProcurementOrder::class, $order->id);
